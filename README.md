## 🙋 과제 활용 노트 — 인공지능과 금융공학 (연세대학교, 2026)

이 저장소는 한국투자증권 KIS Open API 공식 레포지토리를 fork하여, 모의투자 환경에서 삼성전자 자동매매 프로그램을 구현한 과제입니다.

### 구현 목표
- 삼성전자(005930) 종목을 대상으로 모의투자 환경에서 자동매매 흐름 구현
- `ka.auth(svr="vps", product="01")`로 모의투자 계좌 연결
- 현재가 조회 → 조건 판단 → 주문 실행 흐름 구현

### 환경 구성
- GitHub Codespaces에서 Python 환경 구성
- `requirements.txt` 기반 의존성 설치 및 `kis_devlp.yaml` 인증 설정
- KIS 설정 디렉토리 생성: `mkdir -p ~/KIS/config`

### 주요 디버깅
- `auth.py` 접근토큰 발급 시 **403 오류** 발생
- 원인: `Content-Type`이 잘못 설정되어 있었고, 요청 바디 필드명 `appsecret` → `secretkey` 수정 필요
- 전송 방식 `data=` → `json=`으로 변경하여 해결

### 실행 결과 — 삼성전자(005930) 현재가 조회 (2026-06-16)
