## 🙋 과제 활용 노트 
이 저장소는 한국투자증권 KIS Open API 공식 레포지토리를 fork한 것입니다.  
아래는 **인공지능과 금융공학** 수업 과제에서 직접 수행한 작업 내용입니다.

### 환경 구성
- GitHub Codespaces에서 Python 환경을 처음부터 구성
- `requirements.txt` 기반 의존성 설치 및 `kis_devlp.yaml` 인증 설정

### 주요 디버깅
- `auth.py`에서 접근토큰 발급 시 발생한 **403 오류**를 직접 추적
- HTTP 요청 헤더의 필드명 오타(`appkey` → `appKey`)를 찾아 수정하여 인증 성공

### 구현 내용
- 삼성전자(005930) 종목을 대상으로 모의투자 환경에서 **자동매매 프로그램** 구현
